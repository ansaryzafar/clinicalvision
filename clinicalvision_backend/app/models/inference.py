...
# NOTE: MockModelInference and ProductionDemoInference have been permanently removed.
# ClinicalVision is a critical healthcare application — ONLY real AI inference
# via the trained V12 DenseNet-121 ensemble is permitted. No demo, no mock, no fallback.
# The production model weights MUST be deployed on the server.
...
def get_model_inference(version: Optional[str] = None) -> BaseModelInference:
    ...
    Loads the REAL V12 DenseNet-121 production ensemble model.
    No demo mode, no mock fallback — this is a critical healthcare application.
    If the model fails to load, the error is raised so it can be diagnosed and fixed.
    ...
    Raises:
        RuntimeError: If model weights are unavailable or model fails to load
    """
    ...
    if not instance.is_loaded():
        raise RuntimeError(
            "V12 production model created but failed to load. "
            "Check that model weights exist at: "
            f"{model_path}/ensemble/model_*_stage3_best.h5 "
            "and that TensorFlow is properly configured."
        )
    
    _model_instances[model_key] = instance
    logger.info("V12 DenseNet-121 production ensemble loaded and cached successfully")
    return instance
